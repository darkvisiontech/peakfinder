...
tarball.TarReader = class {
  ...
  getTextFile(file_name) {
    let info = this.fileInfo.find((info2) => info2.name == file_name);
    if (info) {
...
tarball.TarWriter = class {
  ...
  _writeChecksum(header_offset) {
    this._writeString("        ", header_offset + 148, 8);
  ...
  _fillHeader(header_offset, opts, fileType) {
    let uid = this._getOpt(opts, "uid", 1e3);
    let gid = this._getOpt(opts, "gid", 1e3);
    ...
    this._writeFileMtime(Math.trunc(mtime / 1e3).toString(8), header_offset);
    this._writeString("ustar", header_offset + 257, 6);
    this._writeString("00", header_offset + 263, 2);
