...
document.addEventListener("DOMContentLoaded", wpd.initApp, true);// Function to detect the OS theme preference
...
wpd.dataTable = (function () {
    ...
    function findIndexOfNotEqual(obj, value) {
        for (let i = 0; i < obj.length; i++) {
            if (obj[i] !== value) {
                return i; // Return the index of the element not equal to the value
            }
        }
        return -1; // Return -1 if all elements are equal to the value
    }
    ...
    function sortRawData() {
        ...
        sortedData = JSON.parse(JSON.stringify(dataCache.rawData));
        var sortingKey = document.getElementById('data-sort-variables').value,
            sortingOrder = document.getElementById('data-sort-order').value,
            isTally = sortingKey === 'Tally',
            peakson = isTally ? selectedDataset.peaksOn : undefined,
        ...
        if (!isConnectivity) {
            if (!isTally) { dataIndex = dataCache.fields.indexOf(sortingKey); }
            else {
                dataIndex = findIndexOfNotEqual(dataCache.fields, peakson);
                tallyindex = dataCache.fields.indexOf(peakson);
            }
            if (dataIndex < 0) {
                return;
            }
            ...
            if (isTally) {
                for (var i = 0; i < sortedData.length; i++) {
                    sortedData[i][tallyindex] = "Joint " + (i + 1);
                    if (peakson == 'Y') { sortedData[i].reverse() }
                }
            }
            return;
        }
    ...
    function makeTable() {
        ...
        var $digitizedDataTable = document.getElementById('digitizedDataTable'),
            ...
            sortingKey = document.getElementById('data-sort-variables').value,
            numFormattingDigits =
        ...
        // Variable Names
        $variableNames.innerHTML = sortingKey == 'Tally' ? 'Tally' : dataCache.fields.join(', ');
...
wpd.algoManager = (function () {
    ...
    function run() {
        ...
        wpd.dataPointCounter.setCount(dataset.getCount());
        wpd.busyNote.close();
        softypo.listeners.w_updateSliderLabel();
        return true;
    }
...
wpd.imageManager = (function () {
    ...
    // function load1p(pngDataUrl) {
    //     _newLoad = true;
    //     let metadata = {
    //         type: "image/png"
    //     };
    //     let file = new File([pngDataUrl], "stacked_pages.png", metadata);
    //     wpd.imageManager.initializeFileManager([file]);
    //     wpd.appData.setPageManager(null);
    //     wpd.imageManager.loadFromFile(file);
    // }
...
wpd.PageManager = class {
    ...
    _initializeInput() {
        ...
        document.getElementById('min-page').value = this.minPage;
        document.getElementById('max-page').value = this.maxPage;
